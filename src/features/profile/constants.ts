export const PROFILE_BACKGROUND = '#f3f4f6';
export const PROFILE_HEADER_BACKGROUND = '#ffffff';
export const PROFILE_CARD_BACKGROUND = '#ffffff';
export const PROFILE_BORDER_COLOR = '#e5e7eb';
export const PROFILE_TEXT_COLOR = '#111827';
export const PROFILE_MUTED_TEXT_COLOR = '#6b7280';
export const PROFILE_ACCENT_COLOR = '#a4ec13';
export const PROFILE_SECTION_GAP = 20;
export const PROFILE_ANIMATION_DURATION_MS = 240;
